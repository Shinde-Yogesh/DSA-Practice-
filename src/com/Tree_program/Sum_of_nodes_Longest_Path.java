...
public class Sum_of_nodes_Longest_Path {
	static class Node {
	 ...
	 public int sumOfLongRootToLeafPath(Node root) {
	        int[] result = new int[2]; // Array to hold maxSum and maxLength
	        findMaxSum(root, 0, 0, result);
	        return result[0];
	    }

	    private void findMaxSum(Node root, int sum, int len, int[] result) {
	        if (root == null) {
	            return;
	        }
	        ...
	        if (root.left == null && root.right == null) {
	            if (len > result[1] || (len == result[1] && sum > result[0])) {
	                result[0] = sum;
	                result[1] = len;
	            }
	            return;
	        ...
	        findMaxSum(root.left, sum, len, result);
	        findMaxSum(root.right, sum, len, result);
	    }
